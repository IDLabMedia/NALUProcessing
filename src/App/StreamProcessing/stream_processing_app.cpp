...
void print_help() {
  ...
  << std::endl
	...
	<< "    <TempId>                       Temporal layers to switch (<TempId from Source, >=TempId from Inject)" << std::endl
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 5 && argc != 6) {
    ...
    std::cout << "TempId should be integer!" << std::endl;
    returnval = -1;
  } else {
    // Open IO files
  ...
  bool injecting = false; //true if we have been writing from inject, false if we have been writing from the source.
  for (int nalu1_idx = 0; nalu1_idx < bufferinputnalu1.size(); ++nalu1_idx) {
    vcl1_increase = false;
    vcl2_increase = false;
    if (bufferinputnalu1[nalu1_idx].type == OTHER) {
